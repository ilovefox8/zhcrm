    ...
    <div class="panel panel-default"<% if (panel.name) print(' data-panel-name="'+panel.name+'"') %>>
        ...
        <% _.each(panel.rows, function (row, rowNumber) { %>
            ...
            <% _.each(row, function (cell, cellNumber) { %>
                <% if (cell != false) { %>
                    <div class="cell cell-<%= cell.name %> <% if (columnCount == 1 || cell.fullWidth == 4) { %>col-sm-12<% } else if (cell.fullWidth == 3) {%> col-sm-9 <% } else if (cell.fullWidth == 2) {%> col-sm-6 <% } else {%> col-sm-3 <% } %> form-group">
